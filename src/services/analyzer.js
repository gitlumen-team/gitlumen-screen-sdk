...
function buildRecommendations(findings, risk) {
  ...
  const base = [
    ...
    'Use this SDK output as initial screening, not as a replacement for human security review.'
  ];
...
export function buildMarkdownReport(report) {
  const lines = [];
  lines.push(`# GitLumen SDK Screening Report`);
  lines.push('');
  ...
  if (report.providerInsights) {
    lines.push(`## Provider Intelligence`);
    lines.push(`**Provider:** ${report.providerInsights.provider}`);
    lines.push(`**Model:** ${report.providerInsights.model || 'unknown'}`);
    if (report.providerInsights.summary) lines.push(report.providerInsights.summary);
    if (report.providerInsights.mergeReadinessRationale) {
      lines.push(`**Merge readiness rationale:** ${report.providerInsights.mergeReadinessRationale}`);
    }
    if (report.providerInsights.riskAdjustments?.length) {
      lines.push(`### Provider Risk Signals`);
      for (const item of report.providerInsights.riskAdjustments) {
        lines.push(`- **[${item.severity}] ${item.title}**${item.file ? ` — \`${item.file}\`` : ''}`);
  ...
  lines.push('');
  lines.push('> Generated by GitLumen Screen SDK. Heuristic and provider-assisted output are screening layers, not replacements for human code/security review.');
  return lines.join('\n');
}
